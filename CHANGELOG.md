...
- **`GET /v1/models` client surface**: always treated as **OpenAI** format (ignores `providerType` / `modelsListFormat` overrides). Use **`GET /anthropic/v1/models`** with base URL `.../anthropic` when the client must see Anthropic-shaped model lists.

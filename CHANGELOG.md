...
- **Cross-protocol upstream paths**: shared rules in [`packages/core/src/converter/crossProtocolUpstreamPath.ts`](packages/core/src/converter/crossProtocolUpstreamPath.ts); [`BodyProcessor`](packages/core/src/server/request/bodyProcessor.ts) applies them whenever `needsConversion`, **before** GET/empty-body return, so **`GET /anthropic/v1/models` → OpenAI upstream `GET /models`** (and symmetric OpenAI-models → Anthropic). [`convertRequestToOpenAI`](packages/core/src/converter/anthropic-to-openai.ts) / [`convertOpenAIRequestToAnthropic`](packages/core/src/converter/openai-to-anthropic-request.ts) use the same helpers for POST path segments.
...
- **Anthropic-prefixed upstream paths**: after **`/anthropic/`**, the proxy now forwards the stripped path unchanged (e.g. **`/anthropic/v1/models` → `baseUrl` + `/v1/models`**). The legacy **`/v1/models` → `/models`** rewrite still applies only for **`/openai/…`** prefixes and unprefixed legacy **`/v1/*`** URLs (OpenAI-style gateways that omit `/v1/` on listings).

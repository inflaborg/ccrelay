...
- **Local HTTP hardening**: `/ccrelay/api/*` requires `Authorization: Bearer <server.apiBearerToken>`. The token is stored under `server.apiBearerToken` in `~/.ccrelay/config.yaml` and is auto-generated on first load if missing. `GET /ccrelay/api/config` returns the token as `"<redacted>"` in the `server` section. The `/ccrelay/` web UI (HTML and assets) additionally requires a fixed internal request header (`X-CCRelay-Internal-UI`), matching the value in `packages/core/src/server/internalUiHeaders.ts`; browsers that open the URL directly receive a small inline info page instead of the SPA. OpenAI-compatible `/v1/*` proxy routes are unchanged. **Desktop (Electron)** injects both the UI header and Bearer for matching `http://127.0.0.1|localhost:<port>/ccrelay/*` via `session.defaultSession.webRequest`. **VS Code/Cursor** webviews inject `window.CCRELAY_API_BEARER` for API calls. **Web dev** (`vite` proxy) must set `CCRELAY_DEV_API_BEARER` to the same token as in `config.yaml` for API calls to succeed.

  ...
  return (
    ...
    >
      <div
        ...
        className="h-full overflow-auto table-scroll"
      >
        {/* Use native table element to avoid nested overflow-auto from Table component */}
        <table style={{ tableLayout: 'fixed', minWidth: '100%' }} className="w-full caption-bottom text-sm">
          <colgroup>
            {columns.map((col) => (
              <col
                key={col.id}
          ...
          <thead className="[&_tr]:border-b sticky top-0 bg-background z-10">
            <tr className="border-b transition-colors hover:bg-muted/50">
              {columns.map((col) => (
                <th
                  key={col.id}
                  className={cn('h-8 px-4 text-left align-middle font-medium text-muted-foreground text-[11px]', col.headerClassName)}
                >
                  {col.header}
                </th>
              ))}
          ...
          <tbody className="[&_tr:last-child]:border-0">
            {data.length === 0 ? (
              <tr className="border-b transition-colors hover:bg-muted/50">
                <td colSpan={columns.length} className="p-4 align-middle h-24 text-center text-xs text-muted-foreground">
                  {emptyMessage}
                </td>
              </tr>
            ) : (
                ...
                {data.map((item, index) => {
                  ...
                  return (
                    <tr
                      ...
                      className={cn('cursor-pointer text-xs border-b transition-colors hover:bg-muted/50', rowClass)}
                      data-last-row={isLast ? 'true' : undefined}
                    >
                      {columns.map((col) => (
                        <td key={`${key}-${col.id}`} className={cn('p-4 align-middle py-1.5 px-2', col.className)}>
                          {col.cell(item, index)}
                        </td>
                      ))}
                    </tr>

...
- Packaged installers are produced under **`packages/desktop/dist/`** (**macOS:** **zip only**—arch in the filename, e.g. `CCRelay-0.2.0-arm64.zip` (no DMG in CI builds; unsigned electron-builder DMGs were invalid UDIF)); **Windows:** NSIS `.exe` with arch, e.g. `CCRelay-0.2.0-x64.exe` and `CCRelay-0.2.0-arm64.exe`. Locally: `npm run desktop:pack:mac` or `desktop:pack:win` **on the host OS**.

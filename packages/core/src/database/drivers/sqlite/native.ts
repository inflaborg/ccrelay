...
export class SqliteNativeDriver implements DatabaseDriver {
  ...
  async getProviderStats(providerId: string, query?: StatsQuery): Promise<ProviderDetailStats> {
    ...
    const base = this.d
      ...
      .get(providerId, providerId, ...timeParams) as Record<string, unknown> | undefined;
    ...
    const modelRows = this.d
      ...
      .all(UNKNOWN_MODEL_LABEL, providerId, ...timeParams, UNKNOWN_MODEL_LABEL) as Record<
      string,
      unknown
    >[];

    const dailyRows = this.d
      .prepare(
      ...
      .all(providerId, ...timeParams) as Record<string, unknown>[];

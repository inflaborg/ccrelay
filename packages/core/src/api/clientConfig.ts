...
model_catalog_json = "${CCRELAY_CODEX_MODEL_CATALOG_FILENAME}"
...
export function buildCodexFields(
  ...
  return [
    ...
    },
    {
      ...
      expected: CCRELAY_CODEX_MODEL_CATALOG_FILENAME,
      current: catalogJson,
      ok: catalogOk,
    },
  ];
  ...
  try {
    ...
    if (body.restore) {
      ...
      if (target === "codex") {
        ...
        // Remove CCRelay-owned model_catalog_json
        let content = lines.join("\n");
        content = removeOwnedCodexModelCatalogJsonField(content);

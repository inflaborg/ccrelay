...
- 可选为指定提供商启用基于 **Tavily** 的 Anthropic 形态 **web search** 工具本地代答；可在 `config.yaml` 或控制台 **Capabilities** 中配置
...
- **Capabilities** — 可选 **Tavily** 联网搜索：API Key、搜索深度、结果条数，以及启用本地代答的提供商列表
...
可选为指定提供商**在本地处理** Anthropic 形态的 **web search**（服务端工具）请求。启用后，CCRelay 通过 **[Tavily](https://tavily.com/)** 搜索 API 执行联网检索，并为该轮对话合成助手回复，上游模型自身无需实现该工具。

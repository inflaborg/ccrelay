...
- **OpenAI 格式转换（LLM 路由）**: 同时支持 Anthropic 与 OpenAI 入口；仅当入站协议与 `providerType` 不一致时做转码，否则透传
...
> 📋 **功能说明**: CCRelay 可同时接受 **Anthropic 与 OpenAI 两类 HTTP 端点**。仅当**入站协议族**与当前 provider 的 `providerType` 不一致时，才做 A↔O 格式转换；若一致则**透传**（仍会做 `modelMap`、鉴权等处理）。
...
- 入站 **OpenAI** + 提供商 `anthropic`：请求 O→A、响应 A→O
- 两侧**协议族相同**时：无格式互转，仅做模型名等映射
...
| 设置 | 默认值 | 描述 |
|------|--------|------|
| `routing.proxy` | `["/v1/messages", "/messages", "/v1/chat/completions", "/v1/models"]` | 路由到当前提供商的路径 |

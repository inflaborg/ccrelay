...
指向 ccrelay 的 OpenAI 客户端通常使用 **`http://127.0.0.1:<端口>/openai`**（形如 `/openai/chat/completions`，无需 `/openai/v1/…`）；或使用 **`http://127.0.0.1:<端口>/v1`**，请求路径为 `/v1/chat/completions` 等。当上游 provider 的 `baseUrl` 已含 `/v1` 时，relay 会做路径对齐以免出现 `/v1/v1/…`。当 **入站 wire 与 `providerType` 不一致**（跨协议）时，**`BodyProcessor`** 会对 **GET 列模型**与 **POST `/v1/messages`** 按与请求转换器相同的规则对齐上游路径（见 [`crossProtocolUpstreamPath.ts`](packages/core/src/converter/crossProtocolUpstreamPath.ts)），而非仅在存在 JSON body 时才处理。
...
| `/v1/models` | GET | OpenAI 模型列表（legacy；协议同 `/openai/models`） |
| `/openai/models` | GET | OpenAI 模型列表 |
| `/anthropic/v1/models` | GET | Anthropic 模型列表 |
...
- **GET models**（`/v1/models`、`/openai/models`、`/anthropic/v1/models`）：**入口路径**决定客户端期望的列表形态（`/v1/models` = OpenAI 形）。**`providerType`** 决定上游 wire。**HTTP 200** 若入口与上游不一致，在满足最小 JSON 结构（OpenAI：`object` 为 list + `data`；Anthropic：含 `data` 数组）时做列表格式转换；否则**原样透传**。**不会因上游错误而用 modelMap 合成列表**——错误状态与原 body 会直接返回（跨协议时在 status ≥400 仍可能套上入口侧常见错误外层）。
...
**GET `/v1/models`** 为 legacy **OpenAI 协议**端点（请使用未带 `/anthropic` 前缀的 base）。需要 Anthropic 形列表时请使用 **`GET /anthropic/v1/models`**（base 以 `/anthropic` 结尾）。成功响应的跨协议转换同上；不再有上游错误时的备用模型列表。

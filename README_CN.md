...
| `routing.block` | `[{path, response, code, condition?}, ...]` | 拦截规则 — 返回自定义响应而非转发。优先于 forward 检查。可选 `condition.kind`（协议名称数组）限制仅对特定客户端协议生效。可选 `condition.providerNot`：仅当**当前** provider ID **不在**列表中时才应用该规则。 |

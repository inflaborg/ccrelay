...
- 可选为指定提供商在本地代答 Anthropic 形态的 **web search** 工具请求，检索后端可选用 **Tavily** 或 **GLM（智谱 Z.ai）**；可在 `config.yaml` 或控制台 **Capabilities** 中配置
...
内置向导与 Cowork 快捷填写会按上述规范 hash 生成 `真实id;展示名;claude-{hash}` 行及对应 `modelMap`。Cowork 请在 Claude Desktop 中配置该请求头；其他环境可不配置。手动编辑 `customModelsList` 后，可在 Provider 编辑器中使用 **重建 modelMap** 同步映射表。
...
**智能路由** Tab 仅用于设置（别名前缀、裸 model id 兜底、排除列表、聚合模型表）。
...
可选为指定提供商**在本地处理** Anthropic 形态的 **web search**（服务端工具）请求。CCRelay 可通过 **[Tavily](https://tavily.com/)** 搜索 API，或通过支持联网的 **GLM（智谱 Z.ai）** 模型端点执行检索，并为该轮对话合成助手回复，上游对话模型自身无需实现该工具。

| 设置                             | 说明                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| `webSearch.enabled`              | 总开关（`true` / `false`）。省略时，非空 `providers` 视为启用。 |
| `webSearch.providers`            | 分配网络搜索的 **提供商 ID**（关闭时仍保留预设列表）。       |
| `webSearch.defaultSearchBackend` | 可选：`tavily` 或 `glm`（未按请求推断时使用的默认后端）。     |
...
| 设置                      | 说明                                                         |
| ------------------------- | ------------------------------------------------------------ |
| `webSearch.glm.apiKey`    | 智谱 API Key。支持 `${ENV_VAR}`。                            |
| `webSearch.glm.endpoint`  | 可选自定义地址；省略时按 `protocol`、`region`、`coding` 推导。 |
| `webSearch.glm.protocol`  | `openai`（Chat Completions）或 `anthropic`（Messages）。     |
| `webSearch.glm.region`    | `intl` 或 `cn`。                                             |
| `webSearch.glm.coding`    | 可选：使用默认端点时是否偏向编程场景主机。                   |
| `webSearch.glm.model`     | 可选模型名（省略时使用内置默认）。                           |

...
- **Settings**: 在 UI 中管理所有 YAML 配置项（日志、并发、服务器、路由）。并发和路由变更即时生效；服务器和日志变更需重启。
...
| `routing.block` | `[{path, response, code, condition?}, ...]` | 拦截规则 — 返回自定义响应而非转发。优先于 forward 检查。可选 `condition.kind`（协议名称数组）限制仅对特定客户端协议生效。 |
...
routing:
  ...
  # 拦截规则：返回自定义响应而非转发。
  # 优先于 forward 检查。condition.kind 按客户端协议过滤。
  # 省略 condition 则对所有协议生效。
  block:
